...
def render_og(title, meta_line, desc):
    ...
    bbox = draw.textbbox((0, 0), "劳伦斯实验室 · 手记", font=brand_font)
    draw.text((46 + lw + 18 - bbox[0], 40 + lh / 2 - 14 - bbox[1]),
              "劳伦斯实验室 · 手记", font=brand_font, fill=CYAN)
...
def collect_notes():
    ...
    for f in sorted(os.listdir(NOTES_DIR)):
        ...
        title = re.sub(r"\s*·\s*(数字殿堂|劳伦斯在上海|劳伦斯实验室).*$", "", tm.group(1)).strip() if tm else m.group(2)

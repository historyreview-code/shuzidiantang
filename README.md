...
- 栏目入口：`/research/`，已接入主导航及首页。
- 报告索引：`src/data/research.ts`；栏目与首页共用 `ResearchCard.astro`。
- 报告正文与PDF：`public/research/<slug>/`，随 Astro 构建原样复制到发布目录。
- 首篇为土地续期与补地价研究，资料截止2026年9月20日，修订于9月22日。
- 新增报告时维护标题、摘要、资料日期、正文与PDF路径；站点地图按索引自动收录。
